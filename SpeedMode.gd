...
onready var Ball = get_node("SpeedBall")
...
func _physics_process(delta):
	...
	if (Ball.position.y <= Line.position.y / 2):
...
func _on_Up_body_entered(_body):
	...
	if PlayerStart == 2:
		...
		PlayerStart = 2
	if score == 9:
		var level = preload("res://WinSpeedMaster.tscn")
...
func _on_Down_body_entered(_body):
	...
	if AIStart == 2:
		...
		AIStart = 2
	if score == 9:
		var level = preload("res://LoseSpeedMode.tscn")
